...
public class JaCMD implements CommandExecutor
{
    ...
    public JaCMD(final Main plugin) {
        super();
        this.plugin = plugin;
        plugin.getCommand("ja").setExecutor((CommandExecutor)this);
    }
        ...
        if (sender instanceof Player) {
            ...
            if (StartKickCMD.voting.contains("true")) {
                if (!StartKickCMD.Ja.contains(p.getName()) && !StartKickCMD.Nein.contains(p.getName())) {
                    StartKickCMD.Ja.add(p.getName());
                    p.sendMessage(String.valueOf(PrefixStartKick) + "§eDu hast für §aJa §eabgestimmt!");
                }
                else {
